...
5. 📝 **更新文档** - 同步相关文档
6. ✅ **代码质量检查** - 执行完整的检查流程 (详见下方)
7. ❓ **询问提交** - "需要我提交这些更改吗？"

### 🔍 提交前必检流程 (Pre-commit Checklist)

**⚠️ 重要：每次提交代码前必须严格按此流程执行检查**
...
#### 前端代码检查 (React + TypeScript)
```bash
cd frontend

# 1. TypeScript类型检查 (必须通过)
pnpm type-check

# 2. ESLint代码规范检查 (修复所有错误)
pnpm lint
...
#### 检查结果要求
- ✅ **cargo fmt**: 无格式化输出 (表示代码已格式化)
- ✅ **cargo fmt -- --check**: 退出码0 (格式符合标准)
- ✅ **cargo clippy**: 无警告或错误输出
- ✅ **cargo test**: 全部测试通过 (70/70)
...
**完成代码修改后:**
1. 📝 总结完成的工作
2. 🔍 **强制执行提交前检查流程** - 按上述Pre-commit Checklist执行
3. ❓ 主动询问: "需要我提交这些更改吗？"
4. ⏳ 等待用户明确授权 ("提交代码"、"commit"等)
5. ✅ 得到授权后执行 git 操作
6. 📋 提交前再次确认提交信息
